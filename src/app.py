...
if pergunta:
    ...
    # Informações em tempo real não disponíveis nesta versão
    elif any(
        termo in pergunta.lower()
        for termo in ["cotação do dólar", "cotacao do dolar", "dólar hoje", "dolar hoje"]
    ):

        resposta = (
            "💡 Nesta versão da NORA, não tenho acesso a cotações "
            "em tempo real. Por isso, prefiro não inventar um valor. "
            "Se você me informar a cotação que deseja usar, posso "
            "fazer simulações e cálculos com ela."
        )
